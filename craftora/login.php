...
// Only allow local relative paths (optionally one folder deep, e.g.
// "admin/index.php") to prevent open-redirect attacks. No "..", no
// leading slash, no external hosts.
if (!preg_match('/^[a-zA-Z0-9_\-]+(\/[a-zA-Z0-9_\-]+)?\.php(\?[a-zA-Z0-9_\-=&%.]*)?$/', $redirect)) {
    $redirect = 'index.php';
}
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($password, $user['password'])) {
                // Regenerate the session ID on every successful login to
